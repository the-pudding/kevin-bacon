...
| `scrolly/story.svelte.js`                                            | The shared interaction state, grouped by interaction (`intro`, `rank`, `race`, `quiz`, `search`, `predict`, `sim`) under four framework fields (`settled`, `entryHeld`, `request`, `running`); `request(kind)`, `resetSimRace()`, `resetGenzLines()`.                      |
...
| `scrolly/search.js`                                                  | The actor search's index: `SEARCH_POOL` (the thousand), `RANK_POOL` (the ladder's 250), `searchActors`, `pathToBacon`, `MAX_PATH_STEPS`, and the one highlight rule the four searchable layouts share (`SEARCH_RGB`, `searchedId`, `withSearchLabel`, `withSearchParams`). |
...
is deliberately sticky across the four charts that can place one (`hopBands`, the
two films scatters, `careerMany`), because the reading is one person carried
through four questions — clearing it on arrival would make the control four
unrelated lookups. It is also the only group whose id the annotation layer cannot
know at build time: see below.
...
return must be in `STATE_TRACKED`, or it has no element to render into — with one
exception, which is why `ScrollyVisual`'s `TRACKED_IDS` is `$derived` rather than
a constant: the searched actor is a thousand possible ids and only ever one at a
time, so it is appended live instead of declared. Declaring the pool would mean a
thousand label elements walked every frame to show one name. `locate(id)` reads
...
— layouts park non-participants hidden, `parkHidden`). `scrolly-nodes.json` also carries `searchPool` (the thousand lowest-rank nodes the
actor search offers) and `searchPaths` (each one's route back to Bacon as
`[costar, film, year]` per hop, from `analysis/bacon-path-tree.py` — the
full-corpus BFS tree with its edges named, sliced here). The build asserts each
route's length equals that actor's `hop`, which is the one disagreement nothing
on screen would report. `scrolly-story.json`

...
| `scrolly/search.js`                                                  | The actor search's index: `SEARCH_POOL` (the recognisable actors plus the story's own cast), `RANK_POOL` (the ladder's 250), `searchActors`, `pathToBacon`, `MAX_PATH_STEPS`, and the one highlight rule the four searchable layouts share (`SEARCH_RGB`, `searchedId`, `withSearchLabel`, `withSearchParams`). |
...
a constant: the searched actor is ~1,400 possible ids and only ever one at a
time, so it is appended live instead of declared. Declaring the pool would mean
all of them walked every frame to show one name. `locate(id)` reads
...
draws, minus anyone the four charts cannot place. Fame has to be borrowed

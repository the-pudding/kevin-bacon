...
| `scrolly/plot.js`, `palette.js`, `cast.js`                           | Plot geometry (`MARGIN`, `TITLE_BAND`, `plotBottom`, `lin`, `Bleed`); the canvas palette (the `mark.*` tokens as rgb, from `$styles/tokens.js`); who is who (named actors, `BY_RANK`, every chart's cast list).                                                                                                                                                                                                                                                                                                                                                                    |
...
- `curve` — a curved state tween out of named origins (`ArrivalCurve`, below);
...
- **A state tween.** `tweener.to(target, ms, jitter, delays)`. Without `delays`
  ...
  snapshots the live buffer, so mashing Next retargets from mid-flight. A
  state that declares a `curve` for the origin (`ArrivalCurve`, `curveFor`)
  hands `to()` a `bows` array built once from the live frame after the out
  beat (`arrivalBows`): each dot's x, y ride a cubic Bézier off the straight
  line, sweeping in at the end and back on it at both ends (a params retarget
  that supersedes the arrival mid-flight keeps its curve, `arrivalCurve` —
  rankFocus's bar lands that way), so the endpoints — and a supersede, which
  never carries bows — are those of a straight tween (motion.md rule 15).
  `edgeLine` draws a live link's near end from the live dot, so a curved
  state with links in flight would swing them; none of the five that curve
  (`STATE_CURVE`) draws one in flight.
...
| race steps                | the frontier column at the race's own y scale, off the canvas where that falls (`placeHiddenDots`); on `raceRecent`/`raceFull` a hidden contender stays on their curve |
...
canvas to its departing spot, on the canvas — or in the canvas's width, off a
race step's frontier column.

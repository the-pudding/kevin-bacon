...
The scrub is the **same render path** as the Stage-3/4 sweep, driven by a **glide loop** rather than an eased clock: entering a scrub (`story.scrubbing` → true) `stop()`s the generic tweeners once (single-writer) and starts one rAF loop (`scrubLoop`) that eases a _rendered_ playhead `renderYear` toward the _input_ target `story.scrubYear` each frame (`renderYear += (target − renderYear) * SCRUB_EASE`, `SCRUB_EASE = 0.22`) and writes `writeRaceSweepFrame(…, scrubFrame(renderYear))` + `drawScene()`. This gives the reel **inertia** so a year change _glides_ instead of snapping — drags chase the finger, and discrete slider-clicks/arrow-keys travel to the target — all one uniform path. It bypasses the reactive layout path (which would route through the straight-line tweener and leak a `layoutFor` cache entry per frame). `scrubFrame(yr)` is a local closure (sibling of `entryFrame`): clip `[yr-SCRUB_HALF, yr]`, domain symmetric `[yr-SCRUB_HALF, yr+SCRUB_HALF]` so the playhead pins to plot-centre and dots move only vertically. The loop runs while scrubbing **or** until the reel catches up after release; once released _and_ settled it writes `story.raceView = scrubView(renderYear)` **once**, and the main effect's `paramChange` branch settles onto the held layout (restarting the generic writer) — same handoff as the Stage-4 landing. **Reduced motion** uses `SCRUB_EASE = 1` (the loop snaps `renderYear` to target in one frame) so year changes jump instantly.

**Next stays clickable:** the glide loop sets `sweeping = true`, so the render effect's existing `sweeping` guard covers it — it yields while the reel is in motion, but a real `stateName` change (Next) `stopSweep()`s the loop, clears `story.scrubbing`, and falls through to a plain tween from the scrubbed buffers (the existing `stateName`→clear-`raceView` effect resets the window). The **Wizard arrow-key collision** is fixed by extending its `keydown` guard to also bail on `el.closest('[role="slider"]')`, so the slider thumb's ArrowLeft/Right step years without triggering step-nav. Furniture (ticks/notes) is gated `{#if !sweeping}` (the glide sets `sweeping`) — hidden during the pan, snapping back for the held frame (per-frame animated furniture remains Stage 6). The three race steps were flipped `ready={false}` → `ready={true}` (chapter visuals are finished; `ready` only gates the prod "visuals tbd" placeholder).
...
- **Year changes glide** (added this session): the glide loop eases `renderYear`→`scrubYear` so slider-clicks/arrow-keys/drag all animate with reel inertia (`SCRUB_EASE = 0.22`, tunable); reduced motion snaps. Verified via CDP canvas-hashing: 7 distinct frames across a discrete jump under no-preference, 2-frame snap under reduce.
- **Deferred (possible follow-ups):** magnetic snap-to-era/preset (`sweepEase`/`runSweepPhase` remain available); a short eased reframe of the _domain shape_ on first grab (year motion now glides, but the initial full-timeline→40yr-window zoom is still a one-frame shape change — `scrubFrame` is year-parameterised only); optionally hiding not-yet-debuted actors' parked dots _during_ the live drag. `SCRUB_HALF` (=20yr) is the tunable zoom half-width; the year readout shows the _target_ year (jumps ahead of the gliding reel on a discrete jump).
